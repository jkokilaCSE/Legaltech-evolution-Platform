...
<div id="dashboardfull" class="bg-white row">
    ...
    <div id="navbar" class="navbar navbar-expand-md navbar-light sticty-top">
        ...
        <div class="">
            <h3 class="text-dark text-center mt-5">Upload Advocates </h3>
        </div>
  ...
  $conn = mysqli_connect("localhost", "root", "","court");
  ...
  if (isset($_POST['submit'])) {
    ...
    while (($fileop = fgetcsv($handle, 100000, ",")) !== false) {
      ...
      $experience = $fileop[7];
      $rateing = $fileop[8];
      $working_day = $fileop[9];
      $working_hour = $fileop[10];
      $description = $fileop[11];
      $wining_percentage = $fileop[12];

      $sql_insert = "INSERT INTO `advocate_detail`(`sno`, `unique_number`, `name`, `address`, `mobile`, `email`, `category`, `location`, `experience`, `rateing`, `working_day`, `working_hour`, `description`, `wining_percentage`) VALUES (NULL,'$unique_number','$name','$address','$mobile','$email','$category','$location','$experience','$rateing','$working_day','$working_hour','$description','$wining_percentage')";
      $pass = mysqli_query($conn,$sql_insert);
    }
    if($pass)
    {
        echo "<script type='text/javascript'> swal('Bulk Upload Success!', 'Advocate data have been successfully uploade', 'success'); </script>";
    }
    else{
        echo "<script type='text/javascript'> swal('Bulk Upload Fail!', 'Advocate data upload have been Failed ', 'error'); </script>";
