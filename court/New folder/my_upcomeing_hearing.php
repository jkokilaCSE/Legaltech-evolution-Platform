...
<div id="dashboardfull" class="bg-white row">
    ...
    <h1 class="text-dark mb-3">Your Upcomeing Hearing</h1>

    <?php
        $case_client_email = $_SESSION["useremail"];
        $sql_select_hearing = "SELECT * FROM `upcomeing_hearing` WHERE `case_client_email`='$case_client_email'";
        $result_hearing = mysqli_query($con,$sql_select_hearing);
        while($data_fetch_upcomeing_hearing=mysqli_fetch_assoc($result_hearing)){
            $case_cnr_number = $data_fetch_upcomeing_hearing["case_cnr_number"];
            $case_acquest_first_name = $data_fetch_upcomeing_hearing["case_acquest_first_name"];
            $case_next_hearing = $data_fetch_upcomeing_hearing["case_next_hearing"];
    ...
    ?>  
            ...
            <div class="row container border border-dark p-3 mt-4">
                <div class="row container-fluid p-1">
                    <div class="col-md">
                        <p class="text-dark">CNR Number : <?php echo $case_cnr_number ?></p>
                    </div>

                    <div class="col-md">
                        <p class="text-dark">Responder Name : <?php echo $case_acquest_first_name ?></p>
                    </div>

                    <div class="col-md">
                        <p class="text-dark">Next Hearing : <?php echo $case_next_hearing ?></p>
                    </div>
                </div>

                <div class="row container-fluid p-1 mt-1">
                    <div class="col-md">
                        <a href="#" class="btn btn-danger form-control mt-1">Hearing Postponded</a>
                    </div>

                    <div class="col-md">
                        <a href="#" class="btn btn-success form-control mt-1">Hearing Completed</a>
                    </div>
