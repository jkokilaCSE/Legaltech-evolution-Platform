...
if(isset($_SESSION["useremail"]))
{
    ...
    $case_select = "SELECT * FROM `case_detail` WHERE `case_advocate_email`='$session_email' AND `case_status`='current'";
    ...
    <div class="container border border-info p-3 mt-2">
        <div class="row">
            <div class="col-md">
            ...
            <div class="col-md">
                <h3 class="text-dark text-center "> <i class="fas fa-eye text-warning"></i> <?php echo $case_status ; ?></h3>
            </div>
        </div>
        <hr>
        ...
        <div class="row">        
            <div class="col-md text-left">
                <a href="edit_my_case.php?case_no=<?php echo $case_no; ?>" class="btn btn-info form-control">Edit Case</a>
            </div>

            
            <div class="col-md text-left">
                <form action="" method="post">
                    <input type="submit" value="Mark as Completed" class="btn btn-success form-control" name='complete_btn'>
                </form>
                ...
                <?php
                    if(isset($_POST["complete_btn"]))
                    {
                        ...
                        $complete_insert = "UPDATE `case_detail` SET `case_status`='$complete_status' where `case_no`='$case_no'";
                        if(mysqli_query($con,$complete_insert))
                        {
                            echo "<script type='text/javascript'> swal('SUCCESS!', 'Your Case Will be Successfully Completed', 'success').then(function(){ window.location='my_case_completed.php'}); </script>";
                        }
                        else{
                            echo "<script type='text/javascript'> swal('Fail!', 'Server Error Please Try Again Later', 'error'); </script>";
                        }
                    }
                ?>
